                ...
                <div class="list-content-title-detail">
                    <div class="list-content-title-detail">
                        ...
                        <div class="list-content-title-pic" v-else>
                            <img width="80" height="80" :src="require('../../../assets/imgs/shopList2.png')" alt/>
                        </div>
                        <div class="list-content-title-project">
                            ...
                            <div class="list-content-title-project-sub-info">
                                <div class="list-content-title-project-sub-info-detail">
                                    {{currSkuInfo.floorNum}}层 {{currSkuInfo.area}}区 {{currSkuInfo.assetNumber}}号 {{currSkuInfo.structureArea?currSkuInfo.structureArea:currSkuInfo.netArea}}平米
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            bigdecimalPrice() {
                return appConst.processingMoneyFormatValueShop(this.currSkuInfo.price)
        ...
        methods: {
                ...
                this.$apiPost(url, params).then((response) => {
                    if (response.data.status === this.$appConst.status) {
                        ...
                        if (content.attachments.length && content.attachments.length > 0) {
                            let attachments = appConst.handleGetListImgUrl(content.attachments, 'pageMap')
                            if (attachments && attachments.length > 0) {
                                this.projectInfo.url = attachments[0].url
                            } else{
                                this.projectInfo.url = require('../../../../src/assets/imgs/shopList2.png')
                            }
                        } else{
                            this.projectInfo.url = require('../../../../src/assets/imgs/shopList2.png')
                        }
            ...
            submitOrder() {
                ...
                let assetsDetails = ''
                assetsDetails = `${this.currSkuInfo.floorNum}层 ${this.currSkuInfo.area}区 ${this.currSkuInfo.assetNumber}号 ${this.currSkuInfo.structureArea?this.currSkuInfo.structureArea:this.currSkuInfo.netArea}㎡`;
