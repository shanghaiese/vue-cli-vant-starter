<template>
    <div class="container">
        ...
        <div>
            <label class="label">手机号(用户名)</label>
        </div>
        <div class="phone-input">
            <input v-model="registerForm.phone" autocomplete="off"/><!--placeholder="请输入手机号"-->
        ...
        <div class="pw-input">
            <input type="text" v-model="verificationCode"  autocomplete="off"/><!--placeholder="请输入验证码"-->
        </div>
        <div>
            <label class="label">姓名</label>
        </div>
        <div class="pw-input">
            <input type="text" v-model="registerForm.realName"  autocomplete="new-password"/><!--placeholder="请输入登录密码"-->
        </div>
        <div>
            <label class="label">登录密码</label>
        </div>
        <div class="pw-input">
            <input type="password" v-model="registerForm.password"  autocomplete="new-password"/><!--placeholder="请输入登录密码"-->
        </div>
        <div>
            <label class="label">确认密码</label>
        </div>
        <div class="pw-input">
            <input type="password" v-model="registerForm.againPassword" autocomplete="new-password"/> <!--placeholder="请再次输入登录密码"-->
        </div>
        <div class="agreement">
            ...
            我已阅读并同意：<span @click="showXieyi=true">《用户注册协议》</span>、<span @click="showZhengce=true">《权易宝隐私政策》</span>
        </div>
        <div class="login-btn">
            <button @click="submitForm">同意协议并注册</button>
        </div>
                            ...
                            <p class="21"
                               style="margin-bottom:7.8000pt;margin-left:49.6000pt;text-indent:-28.3500pt;mso-char-indent-count:0.0000;mso-list:l0 level2 lfo1;">
                                <!--[if !supportLists]--><span
                                    ...
                                    style="mso-spacerun:'yes';font-family:宋体;mso-ascii-font-family:'Times New Roman';mso-hansi-font-family:'Times New Roman';mso-bidi-font-family:'Times New Roman';font-size:12.0000pt;mso-font-kerning:1.0000pt;">”的网站，即权易宝基于信息化、电子化的技术手段为用户提供康养公寓会籍、车位使用权、长租公寓及商业不动产相关权益等资产提供交易平台服务。</span><span
                                    style="mso-spacerun:'yes';font-family:'Times New Roman';mso-fareast-font-family:宋体;font-size:12.0000pt;mso-font-kerning:1.0000pt;"></span>
                            </p>
                            <p class="21"
                               style="margin-bottom:7.8000pt;margin-left:49.6000pt;text-indent:-28.3500pt;mso-char-indent-count:0.0000;mso-list:l0 level2 lfo1;">
    ...
    export default {
        ...
        methods: {
            ...
            handleGetVeriCode() {
                ...
                if (!this.registerForm.phone.length) return this.$global.errMsg('请输入手机号!')
                if (!phoneCodeVerification.test(this.registerForm.phone)) return this.$global.errMsg('手机号格式错误!')
                self.$axios.post(url, {
            ...
            handleCheckSmsMsg() {
                if (this.registerForm.phone == '') return this.$global.errMsg('请填写手机号!')
                if (this.verificationCode == '') return this.$global.errMsg('请填写验证码!')
                if (this.registerForm.realName == '') return this.$global.errMsg('请填写姓名!')
                if (this.registerForm.password == '' || this.registerForm.againPassword == '') return this.$global.errMsg('请填写登录密码或确认密码!')
                if (this.registerForm.password !== this.registerForm.againPassword) return this.$global.errMsg('登录密码和确认密码不一致，请重新填写!')
                let reg = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^]{6,12}$/;
                if (!reg.test(this.registerForm.password)) return this.$global.errMsg('请输入包含大小写字母，数字的6~16位密码!')
                if (!reg.test(this.registerForm.againPassword)) return this.$global.errMsg('请输入包含大小写字母，数字6~16位确认密码!')
                let self = this
                let url = this.$appConst.registerUrl + '/sms/v1.0/checkSmsMsg'
                self.$axios.post(url, {
                    ...
                    if (response.status === 200) {
                        ...
                        if (result) {
                            self.handleSubmitRegister()
                        } else this.$global.errMsg('验证码校验失败！')
                    }
                }).catch(err => {
            ...
            handleSubmitRegister() {
                let self = this
                let url = this.$appConst.userUrl + '/api/v1.0/user/register'
                ...
                self.$axios.post(url, this.registerForm).then(response => {
                    this.count = ''
                    if (response.status == 200){
                        this.$global.errMsg('注册成功!')
